...
allowed-tools: Read, Glob, Grep, Bash(gh issue create *), Bash(gh issue edit *), Bash(gh label create *), Bash(git *), Bash(awk *), Write(gap-reports/**), Edit(gap-reports/**), Edit(.claude/skills/gaps-critical-issue/*), Edit(jaan-to/config/settings.yaml)
...
**Important:** Do NOT use `--body-file` directly on the local copy — the YAML frontmatter will render as visible text in the GitHub issue body. Always strip frontmatter first.

Run via Bash:
```bash
# Strip YAML frontmatter (everything between first two --- lines) before submitting
awk 'BEGIN{n=0} /^---$/{n++; next} n>=2{print}' "gap-reports/{N}-cycle/{N}-issue-report.md" > /tmp/issue-body-clean.md
gh issue create --repo {owner/repo} \
  --title "{title}" \
  --label "enhancement" \
  --body-file /tmp/issue-body-clean.md
rm -f /tmp/issue-body-clean.md
```
...
## Step 9c: Commit Changes

Stage and commit all files written or modified during this session:

```bash
git add "gap-reports/{N}-cycle/{N}-issue-report.md"
git add ".claude/skills/gaps-critical-issue/LEARN.md"  # only if modified during this session
git commit -m "docs(cycle-{N}): create issue report for GitHub issue #{number}

Generated by gaps-critical-issue from {source_report}.
