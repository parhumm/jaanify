...
- Integrate login/auth scaffolds into production apps — 10 operations: 4 new files, 2 replacements, 4 merges (Cycle 14)
- Login page (`/login`): ADHD-friendly Google OAuth with redirect param support, Suspense boundary for Next.js 15 SSG
- Auth middleware: Next.js middleware protecting `/dashboard` and `/tasks/*` with `jaanify_access` cookie check
- Auth store: Zustand in-memory session management with `hydrate()` and `logout()` flows
- NavbarAuth component: auth-aware navbar with avatar dropdown, dashboard link, and sign-out
- API client: cookie-based auth with `withCredentials: true`, automatic `/auth/refresh` on 401
- Backend cookie wrappers: `setAuthCookies()`/`clearAuthCookies()` convenience functions for dual cookie management
- Backend auth routes: cookie-based `/auth/google`, `/auth/refresh`, `/auth/register`, `DELETE /auth/logout`
- Backend auth plugin: dual-mode token extraction (Authorization header + `jaanify_access` cookie)
- RFC 9457 `problemDetailSchema` for typed 401 error responses in auth routes

### Fixed

- Align middleware cookie name with backend — changed from `"access_token"` to `"jaanify_access"` (cookie name mismatch from scaffold)
- Add 401 response schema to auth/refresh route — resolves TS2345 from Fastify type provider constraint
- Wrap login page `useSearchParams()` in `<Suspense>` boundary — required by Next.js 15 for static generation

### Changed

- Landing page hero CTA: secondary action changed from "See How It Works" to "Sign In" linking to `/login`
- Landing page navbar: adds `<NavbarAuth />` component with session hydration on mount
- Upgrade jaan-to context to v7.2.0-1-g3c10276 (Cycle 14)

### Security

- Access token stored in HttpOnly Secure SameSite=Lax cookie (15min TTL) — no longer in localStorage
- Refresh token in HttpOnly Secure SameSite=Strict cookie (7d TTL, path restricted to `/v1/auth`)
- API client removes all localStorage token handling — cookies sent automatically via `withCredentials`
...
### Launch Gap Summary (Updated Cycle 14)
...
| L-30 | P0 | **Resolved** | Login/Auth Integration — scaffolds integrated, build passes |
...
[unreleased]: https://github.com/parhumm/jaanify/compare/v0.5.0...HEAD
[0.5.0]: https://github.com/parhumm/jaanify/compare/v0.4.0...v0.5.0
[0.4.0]: https://github.com/parhumm/jaanify/compare/v0.3.0...v0.4.0
...
> Generated by jaan.to | 2026-02-21 | Skill: release-iterate-changelog | Mode: release v0.5.0 | Cycle: 14 | Status: Updated
