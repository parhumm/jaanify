# Jaanify Entities — Data Model (v7 Regression)

> Generated by jaan.to backend-data-model | 2026-02-16

---

## Executive Summary

7-entity data model for Jaanify on PostgreSQL 16 (Supabase) with UUID primary keys, soft deletes on users and tasks, GDPR-compliant retention (audit log anonymization, guest session TTL), and CHECK-on-VARCHAR enum strategy. Brownfield analysis of existing Prisma v6 schema with all indexes following ESR composite ordering.
...
erDiagram
    ...
    daily_plan_slots {
        ...
        uuid task_id FK "ON DELETE RESTRICT"
...
Existing table — no migration needed. Current Prisma schema matches specification.

```sql
-- Reference DDL (matches current schema.prisma)
CREATE TABLE users (
...
| Name | Columns | Type | Rationale |
|------|---------|------|-----------|
| idx_tasks_on_user_status | (user_id, status) | B-tree (composite) | ESR: equality (user_id, status) for task list filtering |
| idx_tasks_on_deadline | (deadline) | B-tree | Range query for upcoming deadlines |
| idx_tasks_on_user_priority | (user_id, priority_score DESC) | B-tree (composite) | ESR: equality (user_id) + sort (priority DESC) for AI planner |
...
**Schema observation**: The Prisma schema indexes (`idx_tasks_on_user_status`, `idx_tasks_on_deadline`, `idx_tasks_on_user_priority`) do not include `WHERE deleted_at IS NULL` partial index predicates. Prisma v6 does not natively support partial indexes in schema declarations. The C3 output recommended partial indexes; adding them requires raw SQL migration.

**Recommended enhancement** (zero-downtime):

```sql
-- Drop and recreate with partial predicates (CONCURRENTLY)
DROP INDEX CONCURRENTLY idx_tasks_on_user_status;
CREATE INDEX CONCURRENTLY idx_tasks_on_user_status
    ON tasks (user_id, status) WHERE deleted_at IS NULL;

DROP INDEX CONCURRENTLY idx_tasks_on_deadline;
CREATE INDEX CONCURRENTLY idx_tasks_on_deadline
    ON tasks (deadline) WHERE deleted_at IS NULL;

DROP INDEX CONCURRENTLY idx_tasks_on_user_priority;
CREATE INDEX CONCURRENTLY idx_tasks_on_user_priority
...
| idx_feedback_on_task_id | (task_id) | B-tree | FK lookup for task-specific feedback |
...
Append-only tables without `updated_at`: `user_feedback`, `guest_sessions`, `audit_logs`.
...
- Hard delete reserved for GDPR right-to-erasure (after 30-day grace period)

### Multi-Tenancy

Not applicable. Jaanify is single-tenant SaaS. User isolation is via `user_id` foreign keys, not `tenant_id`.
...
Every index on `users` and `tasks` should include `WHERE deleted_at IS NULL` to:
...
### Schema Gap: Partial Index Predicates

The current Prisma schema does not specify partial index predicates on `tasks` indexes (`idx_tasks_on_user_status`, `idx_tasks_on_deadline`, `idx_tasks_on_user_priority`). Prisma v6 does not support `WHERE` clauses in `@@index`. The `users` email unique constraint does use a partial predicate (`idx_users_on_email_active`), likely applied via raw SQL migration.

Recommendation: Apply partial index predicates to tasks indexes via a raw SQL migration (see Tasks migration notes above).
...
|-------|-----------|--------|--------|-------|
| users | EXISTS | N/A | N/A | No migration needed — schema matches |
| tasks | EXISTS + INDEX ENHANCE | Caution | CONCURRENTLY | Add partial predicates to 3 indexes |
| daily_plans | EXISTS | N/A | N/A | No migration needed |
| daily_plan_slots | EXISTS | N/A | N/A | No migration needed |
| user_feedback | EXISTS | N/A | N/A | No migration needed |
| guest_sessions | EXISTS | N/A | N/A | No migration needed |
| audit_logs | EXISTS | N/A | N/A | No migration needed |

**Recommended migration** (partial index enhancement for tasks):

```sql
-- Migration: add_partial_indexes_to_tasks
-- Safety: Caution — use CONCURRENTLY to avoid table locks

BEGIN;
SET lock_timeout = '5s';

DROP INDEX CONCURRENTLY IF EXISTS idx_tasks_on_user_status;
CREATE INDEX CONCURRENTLY idx_tasks_on_user_status
    ON tasks (user_id, status) WHERE deleted_at IS NULL;

DROP INDEX CONCURRENTLY IF EXISTS idx_tasks_on_deadline;
CREATE INDEX CONCURRENTLY idx_tasks_on_deadline
    ON tasks (deadline) WHERE deleted_at IS NULL;

DROP INDEX CONCURRENTLY IF EXISTS idx_tasks_on_user_priority;
CREATE INDEX CONCURRENTLY idx_tasks_on_user_priority
    ON tasks (user_id, priority_score DESC) WHERE deleted_at IS NULL;
...
Note: `CREATE INDEX CONCURRENTLY` cannot run inside a transaction on PostgreSQL. Execute each statement individually outside a transaction block, or use Prisma raw migration with `-- CreateIndex` comments.
...
| Index Coverage | 20% | 4/5 | All FK columns indexed; composite indexes follow ESR rule; partial indexes recommended but not yet in Prisma schema |
...
| Operational Readiness | 15% | 5/5 | Timestamps on all tables; soft delete on users/tasks; GDPR deletion workflow; migration playbook with enhancement recommendations |
| **Overall** | **100%** | **4.8/5.0** | |
...
| Output Path | jaan-to/outputs/backend/data-model/03-jaanify-entities-v7-regression/ |
| Skill | jaan-to:backend-data-model |
| jaan-to Version | v7.0.0 |
| Status | Draft |
| Regression Baseline | 02-jaanify-data-model (v3.19.0, Cycle 3) |
