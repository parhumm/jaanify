# Lessons: jaan-to-dev-stack-detect

> Last updated: 2026-02-03

Accumulated lessons from past executions. Read this before detecting stacks to avoid past mistakes and apply learned improvements.
...
- **Migration in progress**: Multiple versions of same framework detected (e.g., React 17 + React 18) — flag as migration, report both versions
- **Test vs prod dependencies**: devDependencies are dev tools, not production stack. Don't list Jest as a "framework" — it goes under Testing
...
- **Docker dev vs prod**: `docker-compose.override.yml` may have different services than base. Prefer base compose file
...
- Save detection report even when user cancels — useful for future reference
- Read the existing context files BEFORE scanning — knowing what's already filled saves work

## Common Mistakes

Things to avoid based on past feedback:

- Don't overwrite user-added Technical Constraints (these are human decisions, not detectable)
- Don't overwrite Common Patterns section (auth patterns, error handling are team conventions)
- Don't auto-fill monitoring/observability tools (rarely in config files, low confidence)
